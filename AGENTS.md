...
- Permit a composite browser plan only for an explicitly requested ordered outcome on one document lineage. Each step must use an existing strict action schema, bind exact effects and postconditions, and verify its result before the next step. Later steps may use typed references to verified earlier results. Keep native-form registration standalone. Establish `Revision A` on the initial submitted Draft, preserve it for the first Issue, and increment one letter for each later submitted update.
