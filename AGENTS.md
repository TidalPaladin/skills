...
- Keep native-form Draft submission, Issue submission, metadata updates, Version Information updates, and review response completion as separate browser plans. Recheck visible and protected state immediately before each approved submission. Establish `Revision A` on the initial submitted Draft, preserve it for the first Issue, and increment one letter for each later submitted update.
