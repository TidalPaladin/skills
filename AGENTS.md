...
- For `cognidox-qms` writes, use one complete deterministic plan for one defined QMS outcome and obtain approval for its exact plan ID. Combine browser approval with confirmation of the final protected-data transmission. Require `--confirm-notify` for REST issue uploads and `--confirm-destructive` for test cleanup. Never delete a partial record automatically.
...
- Keep Cognidox REST as the first operational channel. For an allowed API gap, use a reusable authenticated browser session. Before approval, permit only read-only discovery and private local artifact preparation. Create the exact browser plan and obtain approval immediately before the first protected-data transmission. Approval authorizes all necessary navigation, field entry, upload, selection, and button clicks for the listed effects. Do not ask again for intermediate UI steps. Never apply a browser plan through REST. Validate every browser-plan section against its action-specific schema so an allowed action cannot carry a prohibited nested operation.
- Permit a composite browser plan only for an explicitly requested outcome on one document lineage. Each step must use an existing strict action schema. Bind exact effects and postconditions. Verify each result before the next step. Later steps may use typed references to verified earlier results. Require the source and destination fields to have the same action-specific type. Use a bound expected-state value for validation. Permit an unbound placeholder only for a documented dynamic string result. Keep native-form registration standalone. Establish `Revision A` on the initial submitted Draft. Preserve it for the first Issue. Increment one letter for each later submitted update.
- Permit a no-notification browser plan only when visible Cognidox state proves that no route exists. Bind the empty Issue user list as applicable. Bind `notificationCapable: false` for Draft submission. Review completion stays at `notify` risk unless both state snapshots bind the same visible disabled-notification statement. Add the exact effect `Do not notify any Cognidox user.` Keep exact-plan and protected-data approval. Keep review and approval requests notification-only. Keep REST Issue uploads at notification risk. REST preflight does not expose authoritative recipient routing.
- For `submit_review_response`, inspect every history entry for the current Draft. Treat `Answer` as the only pending-task indicator for the signed-in reviewer. Ignore `Respond` for status. Bind the part number, Draft version, reviewer identity, and tenant review-page URL or exact task locator. Do not invent a task ID. Require an explicitly directed `updates_required`, `decline_review`, or `accept_review` outcome, the matching visible control, exact success text, and post-submit proof that the result text matches and `Answer` is gone.
- Preserve the page, session handle, local plan, pending plan ID, and completed step results while work or an ambiguous submission remains. Recheck visible state, protected descriptors, recipients, and notification routing before every write. Stop for changed state, a changed protected artifact or recipient set, unexpected notification behavior, a material new UI field or effect, or a prohibited operation. On partial completion, do not roll back or retry blindly. If a completed review reports unexpected email, record it and replace the remaining review plans with `notify` plans.
- Do not infer a Quality decision, MDR determination, CAPA decision, or closure. Do not approve, reject, sign, publish, release, close, make obsolete, or delete through a browser or composite plan. An explicitly directed review outcome is a reviewer response, not a Quality approval or rejection.
- Require `COGNIDOX_QMS_METADATA_ALLOWLIST` for metadata-update browser plans. Bind its exact tenant URL, absolute private file descriptor, and explicit permitted identifier set; fail closed for missing, changed, cross-tenant, or unlisted metadata policy.
