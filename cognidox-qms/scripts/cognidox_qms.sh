...
cognidox_https_origin() {
  local url="$1"
  local jq_bin="$2"

  "${jq_bin}" -enr --arg url "${url}" '
    if ($url | explode | any(. < 32 or . == 127)) then
      error("URL contains control characters")
    else
      $url | capture("^(?<origin>https://[^/?#]+)").origin | ascii_downcase
    end
  ' 2>/dev/null
}

cognidox_require_download_origin() {
  local url="$1"
  local base_url="$2"
  local jq_bin="$3"
  local url_origin
  ...
  if ! url_origin="$(cognidox_https_origin "${url}" "${jq_bin}")" ||
    ! base_origin="$(cognidox_https_origin "${base_url}" "${jq_bin}")"; then
    cognidox_error "version metadata included an invalid download URL."
    return "${COGNIDOX_QMS_EXIT_RUNTIME}"
  fi
  if [[ "${url_origin}" != "${base_origin}" ]]; then
    cognidox_error "version metadata points outside the configured Cognidox origin."
    return "${COGNIDOX_QMS_EXIT_RUNTIME}"
  fi
}
...
cognidox_category_recursive() {
  ...
  local category_id
  local category_file
  local category_pages_file
  local category_page_file
  ...
  while ((${#category_queue[@]} > 0)); do
    ...
    category_pages_file="${temporary_dir}/category-${category_id}-pages.ndjson"
    : >"${category_pages_file}"
    category_offset=0
    while true; do
      category_page_file="${temporary_dir}/category-${category_id}-page-${category_offset}.json"
      status="$(cognidox_api_request "GET" "$(cognidox_query_with_filters "/categories/$(cognidox_urlencode "${category_id}")" "${category_offset}" "${COGNIDOX_QMS_CATEGORY_PAGE_SIZE}" details categories documents)" "${category_page_file}" "" "${curl_bin}" "${token_value}" "${base_url}")"
      if [[ ! "${status}" =~ ^2[0-9][0-9]$ ]]; then
        break
      fi
      "${jq_bin}" -c . "${category_page_file}" >>"${category_pages_file}"
      page_category_count="$("${jq_bin}" -r '.categories // [] | length' "${category_page_file}")"
      page_document_count="$("${jq_bin}" -r '.documents // [] | length' "${category_page_file}")"
      if ((page_category_count < COGNIDOX_QMS_CATEGORY_PAGE_SIZE && page_document_count < COGNIDOX_QMS_CATEGORY_PAGE_SIZE)); then
        break
      fi
    ...
    if [[ "${status}" =~ ^2[0-9][0-9]$ ]]; then
      "${jq_bin}" -s '
        reduce .[] as $page (
          {details: null, categories: [], documents: []};
          .details = (.details // $page.details) |
          .categories += ($page.categories // []) |
          .documents += ($page.documents // [])
        )
      ' "${category_pages_file}" >"${category_file}"
...
cognidox_download_version() {
  ...
  for ((index = 0; index < link_count; index++)); do
    ...
    cognidox_require_download_origin "${href}" "${base_url}" "${jq_bin}"
...
cognidox_main() {
  ...
  local base_url="${COGNIDOX_QMS_BASE_URL:-}"
  ...
  case "${mode}" in
    ...
    version)
      ...
      if [[ "${download_version}" == "true" ]]; then
        cognidox_download_version "${body_file}" "${output_path}" "${temporary_dir}" "${curl_bin}" "${jq_bin}" "${cognidox_token}" "${base_url}"
      else
        cognidox_render_response "${mode}" "${output_format}" "${body_file}" "${jq_bin}"
      fi
      ;;
