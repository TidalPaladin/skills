...
case "${method} ${url}" in
  ...
  "GET https://mock.cognidox.example/api/v1.0/categories?filter=details&filter=categories&filter=documents&limit=5")
    ...
    ;;
  "GET https://mock.cognidox.example/api/v1.0/categories?filter=details&filter=categories&filter=documents")
    response_body='{"details":{"id":0,"name":"Root","canCreateDocuments":false},"categories":[{"id":10,"name":"Quality","categoryCount":26,"documentCount":0}],"documents":[]}'
    ;;
  "GET https://mock.cognidox.example/api/v1.0/categories/10?filter=details&filter=categories&filter=documents&limit=25"|\
  "GET https://mock.cognidox.example/api/v1.0/categories/10?filter=details&filter=categories&filter=documents&offset=0&limit=25")
    response_body="$(jq -cn '{details:{id:10,name:"Quality"},categories:[range(11;36) | {id:.,name:("Category " + tostring),categoryCount:0,documentCount:0}],documents:[]}')"
    ;;
  "GET https://mock.cognidox.example/api/v1.0/categories/10?filter=details&filter=categories&filter=documents&offset=25&limit=25")
    response_body='{"details":{"id":10,"name":"Quality"},"categories":[{"id":36,"name":"Category 36","categoryCount":0,"documentCount":0}],"documents":[]}'
    ;;
  ...
  "GET https://mock.cognidox.example/api/v1.0/documents/versions/DM-000401-AN/1A?format=pdf&sliceSize=10")
    if [[ "${MOCK_CROSS_ORIGIN_LINK:-false}" == "true" ]]; then
      response_body='{"partNumber":"DM-000401-AN","version":"1A","length":19,"links":[{"href":"https://files.example.invalid/document-slice","rel":"slice-0"}]}'
    else
...
run_with_mock() {
  ...
  run_command env \
    ...
    MOCK_CROSS_ORIGIN_LINK="${MOCK_CROSS_ORIGIN_LINK:-false}" \
    "${COGNIDOX_SCRIPT}" --base-url https://mock.cognidox.example/api/v1.0 "$@"
}

run_with_mock_traced() {
  local secret_dir="$1"
  local mock_curl="$2"
  local log_file="$3"
  shift 3
  ...
  run_command env \
    ...
    --base-url https://mock.cognidox.example/api/v1.0 "$@"
}
...
run_tests() {
  ...
  run_command env \
    ...
    COGNIDOX_CURL_BIN="${mock_curl}" \
    EXPECTED_TOKEN="${SECRET_SENTINEL}" \
    "${COGNIDOX_SCRIPT}" --repository
  assert_equals "2" "${LAST_EXIT_CODE}" "base URL should be required"
  assert_contains "${LAST_STDERR}" "--base-url or COGNIDOX_QMS_BASE_URL" "missing base URL should explain configuration"
  ...
  run_with_mock_traced "${secret_dir}" "${mock_curl}" "${log_file}" --auth-smoke-test
  assert_equals "0" "${LAST_EXIT_CODE}" "traced auth smoke should succeed"
  assert_not_contains "${LAST_STDERR}" "${SECRET_SENTINEL}" "xtrace stderr should not reveal token"
  ...
  : >"${log_file}"
  run_with_mock "${secret_dir}" "${mock_curl}" "${log_file}" --category-root --recursive --max-categories 1
  assert_equals "0" "${LAST_EXIT_CODE}" "recursive category traversal should succeed"
  assert_contains "${LAST_STDOUT}" "category id=10 status=200 child_categories=26" "recursive traversal should aggregate category pages"
  assert_contains "$(<"${log_file}")" "offset=25&limit=25" "recursive traversal should request the second category page"
  ...
  assert_contains "${LAST_STDERR}" "outside the configured Cognidox origin" "cross-origin failure should explain the safety boundary"
